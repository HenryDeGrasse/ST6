...
export const PlanDrillDown: React.FC<PlanDrillDownProps> = ({
  ...
  if (loading) {
    return (
      <div data-testid="drilldown-loading" aria-busy="true" className={styles.skeletonWrapper}>
  ...
  if (error) {
    return (
      <div data-testid="drilldown-error" className={styles.errorWrapper}>
        ...
        <button onClick={onBack} className={styles.errorBackBtn}>
          ← Back
        </button>
      </div>
    );
  ...
  const canReview = plan.state === PlanState.RECONCILED || plan.state === PlanState.CARRY_FORWARD;
    ...
    <div data-testid="plan-drilldown" className={styles.container}>
      ...
      <div className={styles.planHeader}>
        ...
        <p className={styles.planMeta}>
          Week: {plan.weekStartDate} · State: <strong>{plan.state}</strong> · Review:{" "}
          <strong>{plan.reviewStatus}</strong>
          {plan.lockType && (
            <>
              {" "}
              · Lock: <strong>{plan.lockType}</strong>
            </>
          )}
        </p>
      </div>
        ...
        <div className={styles.tableWrap}>
          <table data-testid="drilldown-commits" className={styles.table}>
            ...
            <tbody>
              {commits.map((c) => {
                const statusClass =
                  showActuals && c.actual?.completionStatus ? COMPLETION_ROW_CLASS[c.actual.completionStatus] : "";
                return (
                  <tr
                    key={c.id}
                  ...
                  >
                    <td className={styles.td}>
                      {c.title}
                      {c.carriedFromCommitId && <span className={styles.carriedBadge}>(carried)</span>}
                    </td>
                    ...
                    <td className={styles.td}>
                      ...
                      {c.nonStrategicReason && (
                        <span className={styles.nonStrategicText}>Non-strategic: {c.nonStrategicReason}</span>
                      )}
                    </td>
                    {showActuals && (
                      <>
      ...
      {canReview && <ReviewPanel plan={plan} onSubmitReview={onSubmitReview} loading={loading} />}
    </div>
  );
};
