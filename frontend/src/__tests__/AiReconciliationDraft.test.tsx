...
describe("AiReconciliationDraft", () => {
  ...
  it("calls onFetchDraft when button is clicked", () => {
    ...
    renderWithFlags(<AiReconciliationDraft {...defaultProps} onFetchDraft={onFetchDraft} />);
    fireEvent.click(screen.getByTestId("ai-draft-fetch"));
    expect(onFetchDraft).toHaveBeenCalled();
  });

  it("shows loading state", () => {
    renderWithFlags(<AiReconciliationDraft {...defaultProps} draftStatus="loading" />);
    expect(screen.getByTestId("ai-draft-loading")).toBeInTheDocument();
  });

  it("shows unavailable state", () => {
    renderWithFlags(<AiReconciliationDraft {...defaultProps} draftStatus="unavailable" />);
    expect(screen.getByTestId("ai-draft-unavailable")).toBeInTheDocument();
  });

  it("renders draft items when ok", () => {
    const item = makeDraftItem();
    renderWithFlags(<AiReconciliationDraft {...defaultProps} draftStatus="ok" draftItems={[item]} />);
  ...
  it("calls onApplyDraft when Apply is clicked", () => {
    ...
    const item = makeDraftItem();
    renderWithFlags(
      <AiReconciliationDraft {...defaultProps} draftStatus="ok" draftItems={[item]} onApplyDraft={onApplyDraft} />,
    );
  ...
  it("labels panel as beta", () => {
    renderWithFlags(<AiReconciliationDraft {...defaultProps} draftStatus="ok" draftItems={[makeDraftItem()]} />);
